...
export function UserModeProvider({ children }: { children: React.ReactNode }) {
  ...
  const [signMode, setSignMode] = useState(false);
  const [touchVoiceMode, setTouchVoiceMode] = useState(false);
  const [emotionType, setEmotionTypeState] = useState<EmotionType>(null);
  ...
  // ✅ Enhanced: Responds to emotion changes like "stressed"
  const setEmotionType = (e: EmotionType) => {
    setEmotionTypeState(e);
    if (e === "stressed") {
